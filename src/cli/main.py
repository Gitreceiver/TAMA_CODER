...
import git_utils
from src.agent.agent_loop import AgentLoop # Import AgentLoop
import src.ai.tools.filesystem # Import to register tools
...
@app.command(name="do", help="🔥 Core function: AI automatically executes the specified task.")
async def do_task(
    ...
    # 2. 启动引擎
    engine = AgentLoop(task_id=task_id)
    success = await engine.run()

    # 3. 善后处理
    if success:
        if auto or typer.confirm("任务已完成，是否提交代码并更新任务状态？"): # Use auto flag
            commit_message = f"feat: Complete task #{task.id} {task.title}"
            git_utils.commit_changes(commit_message)
            core.set_task_status(tasks_data.tasks, task_id, "done")
            save_task_data(tasks_data)
            ui.console.print("[green]✅ 流程结束[/green]")
        else:
            ui.console.print("[yellow]流程中断，未提交代码和更新任务状态。[/yellow]")
    else:
        ui.console.print("[red]任务未成功完成。代码未提交，任务状态未更新。[/red]")
...
def remove_command(
    ...
    for tid in task_ids:
        if ',' in tid:
            parsed_task_ids.extend([d.strip() for d in tid.split(',') if d.strip()])
        else:
...
def parse_prd_command(
    ...
    try:
        ...
        if success:
            ui.console.print(Panel("Successfully parsed PRD and saved tasks to SQLite.", title="[bold green]✅ PRD Parsed[/bold green]", border_style="green"))
        else:
...
@app.command(name="chat", help="Start an AI-powered interactive coding session for a task.")
def chat_command(
    task_id: str = typer.Argument(..., help="The ID of the task to work on."),
):
    """Starts an interactive AI coding session for a specified task."""
    ui.console.print(f"[bold green]Starting AI chat session for Task #{task_id}...[/bold green]")
    ...
    agent_loop = AgentLoop(task_id=task_id) # Instantiate AgentLoop
    # In a real async CLI, you'd run this with an async runner, e.g., anyio.run(agent_loop.run)
    # For now, we'll keep it synchronous as the AgentLoop.run is still a placeholder
    import asyncio
    asyncio.run(agent_loop.run())

    ui.console.print(f"[bold green]AI chat session for Task #{task_id} ended.[/bold green]")

async def _async_cli_entry_point():
    # Ensure tasks directory exists (optional, storage might handle this)
    tasks_dir = os.path.dirname(settings.TASKS_DIR_PATH)
...
if __name__ == "__main__":
    asyncio.run(_async_cli_entry_point())
