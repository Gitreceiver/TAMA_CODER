...
from task_manager.data_models import Task, Subtask, Status, Priority, Dependency, TasksData, MetaData
from task_manager import storage_sqlite as storage
...
# Load initial tasks from SQLite
tasks_data = storage.load_tasks()
tasks_list: List[Task] = tasks_data.tasks
meta: MetaData = tasks_data.meta
logger.info(f"Loaded {len(tasks_list)} tasks from SQLite database")

# Helper to save tasks after modification
def _save_current_tasks():
    try:
        tasks_data = TasksData(meta=meta, tasks=tasks_list)
        storage.save_tasks(tasks_data)
        logger.debug(f"Saved {len(tasks_list)} tasks to SQLite")
        return True
    except Exception as e:
        logger.error(f"Failed to save tasks to SQLite: {e}", exc_info=settings.DEBUG)
        return False
